...
/***/ function(module, exports) {
	...
	function loadBooks() {
	    ...
	    for (var _i = 0; _i < data.length; _i++) {
	        ...
	        values = [data[_i].id, data[_i].type, data[_i].author.name, data[_i].title, data[_i].branch, data[_i].publishingHouse];
	...
	function loadAudiobooks() {
	    ...
	    for (var _i2 = 0; _i2 < data.length; _i2++) {
	        if (data[_i2].type == "Audiobook") {
	            row = document.createElement("tr");
	            tbody.appendChild(row);
	            values = [data[_i2].id, data[_i2].type, data[_i2].author.name, data[_i2].title, data[_i2].branch, data[_i2].publishingHouse];
	            for (var j = 0; j < values.length; j++) {
	...
	function loadTextbooks() {
	    ...
	    for (var _i3 = 0; _i3 < data.length; _i3++) {
	        if (data[_i3].type == "Textbook") {
	            row = document.createElement("tr");
	            tbody.appendChild(row);
	            values = [data[_i3].id, data[_i3].type, data[_i3].author.name, data[_i3].title, data[_i3].branch, data[_i3].publishingHouse];
	            for (var j = 0; j < values.length; j++) {
...
/***/ function(module, exports) {
	...
	function create(callType) {
	    ...
	    if (callType == 'ab') {
	        ...
	        var narrator = document.getElementById('narrator').value;
	        var type = "Audiobook";
	        var tmp = "type=" + encodeURIComponent(type) + "&title=" + encodeURIComponent(title) + "&authorName=" + encodeURIComponent(author) + "&publishingHouse=" + encodeURIComponent(publishingHouse) + "&narrator=" + encodeURIComponent(narrator) + "&duration=" + encodeURIComponent(duration) + "&branch=" + encodeURIComponent(branch);
	    }
	    if (callType == 'tb') {
	        var numofpage = document.getElementById('numofpage').value;
	        var binding = document.getElementById('binding').value;
	        var _type3 = "Textbook";
	        var tmp = "type=" + encodeURIComponent(_type3) + "&title=" + encodeURIComponent(title) + "&authorName=" + encodeURIComponent(author) + "&publishingHouse=" + encodeURIComponent(publishingHouse) + "&numberOfPage=" + encodeURIComponent(numofpage) + "&binding=" + encodeURIComponent(binding) + "&branch=" + encodeURIComponent(branch);
	    }
