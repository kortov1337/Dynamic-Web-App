...
export class Book{
    constructor (title,publishingHouse,type,branch,id,releaseDate){
    this.id=id;
    this.title=title;
...
function loadBooks(){
    ...
    let values = ["ID","Type","Author Name","Title","Branch","Publishing House","Release Date",""];
     ...
     for(let i=0; i<data.length; i++)
    {
        ...
        values=
        [data[i].id, data[i].type, data[i].author.name, data[i].title, data[i].branch, data[i].publishingHouse, data[i].releaseDate];
